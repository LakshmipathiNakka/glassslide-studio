...
export const PropertiesPanel = ({ selectedElement, onElementUpdate, onElementDelete }: PropertiesPanelProps) => {
  ...
  const renderThemePreview = (theme: TableTheme) => {
    ...
    return (
      <motion.div
        key={theme.id}
        className={`relative rounded-lg p-2 border-2 transition-all duration-200 cursor-pointer group ${isSelected
      ...
      >
        ...
        <div className="mt-1.5 text-center">
          <div className={`text-[11px] font-medium transition-colors ${isSelected ? 'text-blue-700' : 'text-gray-700 group-hover:text-gray-900'
            }`}>
            {theme.name}
          </div>
          {/* Color indicators */}
                                    ...
                                    <div key={colorIndex} className="relative group">
                                      <div className="relative">
                                        <button
                                          ...
                                          disabled={isDisabled}
                                          className={`w-8 h-8 rounded-lg border-2 transition-all duration-200 ${isDisabled
